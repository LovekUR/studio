...
export default function DifferentiatedMaterialsPage() {
  const [generatedWorksheets, setGeneratedWorksheets] = useState<GeneratedWorksheets | null>(null);
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast({
        ...
        description: "Failed to generate worksheets. Please check your input and try again.",
      });
    } finally {
      setIsLoading(false);
    }
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <>
                          ...
                          <label htmlFor="picture" className="cursor-pointer">
                            <div className="border-2 border-dashed border-muted-foreground/50 rounded-lg p-6 text-center hover:border-primary transition-colors">
                              {preview ? (
                                <Image src={preview} alt="Preview" width={400} height={300} className="mx-auto rounded-md object-contain max-h-48" />
                              ) : (
                                <div className="flex flex-col items-center">
                                  <UploadCloud className="w-12 h-12 text-muted-foreground" />
                                  <p className="mt-2 text-muted-foreground">Click to upload or drag & drop</p>
                                </div>
                              )}
                            </div>
                          </label>
                ...
                <Button type="submit" disabled={isLoading} className="w-full">
                  {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Generate Worksheets"}
                </Button>
              </form>
            </Form>
          </CardContent>
          ...
          <CardContent className="min-h-[300px]">
            ...
            {generatedWorksheets ? (
               <Accordion type="single" collapsible className="w-full">
                 {Object.entries(generatedWorksheets).map(([grade, content]) => (
                   <AccordionItem value={`item-${grade}`} key={grade}>
                     <AccordionTrigger className="font-headline capitalize">Worksheet for {grade}</AccordionTrigger>
            ...
            ) : !isLoading && <p className="text-muted-foreground">Your generated worksheets will appear here, organized by grade level.</p>}
          </CardContent>
        </Card>
      </div>
    </FeaturePage>
